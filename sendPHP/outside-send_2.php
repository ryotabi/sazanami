...
$data = file_get_contents('outside-send_1.txt'); /*toukou_1.txtを読み込む*/
$element = explode( "\n",$data);/*toukou_1.txtを各行で分割する。*/
$cnt = count( $element ); /*分割した要素の数（行数）をカウントする*/

    for( $i=0;$i<$cnt;$i++ ){ /*行末までループする*/

    $fp = fopen("outside-send_2.txt","a"); /*追記モードで開く*/
    $num = count( file('outside-send_2.txt')); /*ファイルのデータの行数をかぞえて＄numに代入*/
    $num++; /*投稿番号の取得*/
